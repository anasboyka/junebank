...
class DuitnowFormDetail extends StatefulWidget {
  Map argument = {};
  DuitnowFormDetail({Key key, this.argument}) : super(key: key);
  @override
  _DuitnowFormDetailState createState() => _DuitnowFormDetailState();
}

class _DuitnowFormDetailState extends State<DuitnowFormDetail> {
                          ...
                          child: TextButton(
                            ...
                            onPressed: () async {
                              ...
                              if (amount.isEmpty || double.parse(amount) < 1) {
                                ...
                                if (id != null) {
                                  loading = false;
                                  String accountType = await getdataUser(
                                      'account',getCurrentUserId(), 'accountType');
                                  int accountNumber = await getdataUser(
                                      'account',getCurrentUserId(), 'accountNumber');
                                  int recipientAC =
                                      await getdataUser('account',id, 'accountNumber');
                                  String recipientAN =
                                      await getdataUser('account',id, 'accountName');
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text(
                                          'account for phone number not exist'),
                                    ),
                                  );
                                }
                              }
        ...
        children: [
          ...
          IntrinsicWidth(
            child: TextFormField(
              ...
              //     return val.isEmpty?"please enter account number":null;
